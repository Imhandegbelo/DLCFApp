...
const FileUpload = (req, res) => {
  ...
  uploadFile(audioFile, fileName)
    .then(({ filePath, originalName }) => {
...
const uploadFile = (imageFile, fileName) => {
  return new Promise((resolve, reject) => {
    ...
    imageFile.mv(
      path.join(
        __dirname,
        `../public/${(fileName, newFilename)}-${imageFile.name}`
      ),
      function (err) {
        if (err) {
          reject(err);
        ...
        resolve({
          filePath: `public/${(fileName, newFilename)}-${imageFile.name}`,
...
const DownloadParticipantList = async (req, res) => {
  try {
    ...
    const formattedParticipants = participants.map((participant, i) => {
      if (participant._doc) {
        ...
        participantDetail["Email"] = !Number(participant._doc.email)
          ? participant._doc.email
          : "";
        ...
        participantDetail["Date Registered"] = moment(
          participant._doc.created
        ).format("MMM Do YY");
...
const DownloadFeedbackList = async (req, res) => {
  try {
    ...
    const formattedFeedbackList = feedbacks.map((feedback, i) => {
      if (feedback._doc) {
        ...
        feedbackDetail["What Programme touched you the most"] =
          feedback._doc.message;
        ...
        feedbackDetail["What should be improved in the camp"] =
          feedback._doc.improvements;
...
const DownloadTestimonyList = async (req, res) => {
  try {
    ...
    const formatedTestimonyList = testimonies.map((testimony, i) => {
      if (testimony._doc) {
        ...
        testimonyDetail["Date Submitted"] = moment(
          testimony._doc.created
        ).format("H:MM Do MMM YY");
